...
```{r housing}
load.libraries <- c('data.table', 'testthat', 'gridExtra', 'corrplot', 'GGally', 'ggplot2', 'e1071', 'dplyr', 'DT', 'tidyr', 'purrr', 'mi')
sapply(load.libraries, require, character = TRUE)
```


## Load train/test Data
If you have not used *fread* - it's a way to read in a file directly to a data.table as opposed to a data.frame. 
```{r, echo = FALSE}
...
Track the catagorical feature names separately from the numeric feature names.
```{r}
cat_var <- names(train)[which(sapply(train, is.character))]
numeric_var <- names(train)[which(sapply(train, is.numeric))]
```

## Structure of the data {.tabset}
The housing data set has `r dim(train)[1]` rows and `r dim(train)[2]` features with the target feature Sale Price.
Note the structure of the data in the next tab - treating all factors as characters for now. 
...
## Convert to Factors / Separate Fact from Numeric {.tabset}

### Convert and Create
Convert characters to factors and create training sets for catagorial and numerical data.
```{r}
...
**Note* Create function to iterate next. **
```{r}
train_cont[,1:6] %>%
...
### Missing Data2
This plot shows the intersection of missing data as well as the populations of missing data.
```{r}
library(naniar)
library(UpSetR)
var_w_miss_data <- n_var_miss(train)
